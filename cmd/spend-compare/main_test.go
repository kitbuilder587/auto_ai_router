...
func TestExecuteReturnsNonZeroOnDivergence(t *testing.T) {
	...
	compare := func(context.Context, string, string, spendcompare.Filter) (spendcompare.Report, error) {
		return spendcompare.Report{Equal: false}, nil
	}
...
func TestExecuteReturnsZeroOnEqualReport(t *testing.T) {
	...
	compare := func(context.Context, string, string, spendcompare.Filter) (spendcompare.Report, error) {
		return spendcompare.Report{Equal: true}, nil
	}
...
func TestExecuteRequiresUTCWindowAndDSNs(t *testing.T) {
	...
	compare := func(context.Context, string, string, spendcompare.Filter) (spendcompare.Report, error) {
		...
		return spendcompare.Report{}, nil
	}
...
func TestExecuteNeverPrintsDSNs(t *testing.T) {
	...
	compare := func(_ context.Context, gotTest, gotReference string, _ spendcompare.Filter) (spendcompare.Report, error) {
		if gotTest != testDSN || gotReference != referenceDSN {
			t.Fatalf("DSNs were not passed to comparer")
		}
		return spendcompare.Report{}, errors.New("failed using " + gotTest + " and " + gotReference)
	}
