...
type compareFunc func(context.Context, string, string, shadowcompare.Filter) (shadowcompare.Report, error)

func main() {
	os.Exit(execute(os.Args[1:], os.Stdout, os.Stderr, os.Getenv, shadowcompare.CompareDatabases))
}
...
type cliOptions struct {
	testDSN      string
	referenceDSN string
	filter       shadowcompare.Filter
	timeout      time.Duration
}

func parseOptions(args []string, getenv func(string) string) (cliOptions, error) {
	flags := flag.NewFlagSet("shadow-compare", flag.ContinueOnError)
	...
	from, err := shadowcompare.ParseUTC(fromValue)
	if err != nil {
		return cliOptions{}, fmt.Errorf("invalid --from: %w", err)
	}
	to, err := shadowcompare.ParseUTC(toValue)
	if err != nil {
		return cliOptions{}, fmt.Errorf("invalid --to: %w", err)
	}
	window, err := shadowcompare.NewWindow(from, to)
	if err != nil {
