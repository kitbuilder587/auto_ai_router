...
`spend_log.database_url`. When this destination is configured, it is
authoritative: startup and readiness fail closed if billing, auth, pricing, or
atomic enforcement are unavailable. Startup also checks
`expected_database_name` so a configuration mistake cannot silently write to
an unintended database.

## Comparison-window validity

`auto_ai_router_spend_comparison_window_valid` is a conservative,
...
## Comparison semantics (what spend-compare can and cannot prove)
