...
func TestBuildMetadata(t *testing.T) {
	t.Run("nil_tokenInfo", func(t *testing.T) {
		result := buildMetadata("hashed123", nil, "", 0, nil)
	...
	t.Run("with_tokenInfo_and_aliases", func(t *testing.T) {
		...
		result := buildMetadata("hashed456", tokenInfo, "", 0, nil)
		var m map[string]interface{}
		err := json.Unmarshal([]byte(result), &m)
		require.NoError(t, err)
	...
	t.Run("with_error_info", func(t *testing.T) {
		result := buildMetadata("hashed789", nil, "rate limit exceeded", http.StatusTooManyRequests, nil)
		var m map[string]interface{}
		err := json.Unmarshal([]byte(result), &m)
		require.NoError(t, err)
