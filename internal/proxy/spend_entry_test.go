...
func TestBuildSpendEntryPreservesSignedOriginalCallTypeAfterRouteTranslation(t *testing.T) {
	...
	entry := p.buildSpendEntry(&RequestLogContext{
...
func TestBuildSpendEntryUsesCanonicalBillingAndSignedIdentity(t *testing.T) {
	registry := models.NewModelPriceRegistry()
	registry.Update(map[string]*models.ModelPrice{
		"provider-model": {
...
func TestBuildSpendEntryUsesAuthenticatedTokenInfoForDirectAIR(t *testing.T) {
	registry := models.NewModelPriceRegistry()
	registry.Update(map[string]*models.ModelPrice{
		"provider-model": {InputCostPerToken: 0.000001, OutputCostPerToken: 0.000004},
	})
...
func TestBuildSpendEntryDirectAIRPersistsSafeRequestFieldsAndSelectedDeployment(t *testing.T) {
	logger := slog.New(slog.DiscardHandler)
	...
	fallbackEntry := p.buildSpendEntry(logCtx)
...
func TestBuildSpendEntryUsesUniquePublicDeploymentAcrossOuterCredential(t *testing.T) {
	logger := slog.New(slog.DiscardHandler)
	modelManager := models.New(logger, 100, nil)
...
func TestBuildSpendEntrySignedIdentityKeepsDeploymentAndTagsAuthoritative(t *testing.T) {
	logger := slog.New(slog.DiscardHandler)
	modelManager := models.New(logger, 100, nil)
...
func TestBuildSpendEntryFallbackUsesCapturedPublicModel(t *testing.T) {
...
func TestBuildSpendEntryWritesEmptyTagArrayForDirectAIR(t *testing.T) {
	p := &Proxy{logger: slog.New(slog.DiscardHandler)}
	billing := NewBillingContext("event-direct", "call-direct", "/v1/chat/completions", shadowcontext.Identity{}).
		WithRouting("backend-model", "provider-model", "openai", "credential-1", "https://provider.example.invalid/v1")
	entry := p.buildSpendEntry(&RequestLogContext{
...
func TestBuildSpendEntryPricesImagesWithoutProviderUsage(t *testing.T) {
	registry := models.NewModelPriceRegistry()
	registry.Update(map[string]*models.ModelPrice{
...
func TestBuildSpendEntryPricesImageEndpointOutputTokensAtImageRate(t *testing.T) {
	registry := models.NewModelPriceRegistry()
	registry.Update(map[string]*models.ModelPrice{
		"gpt-image-token": {
			InputCostPerToken:       0.000001,
...
func TestBuildSpendEntryPreservesOpenAIChatFailureRoute(t *testing.T) {
	registry := models.NewModelPriceRegistry()
	registry.Update(map[string]*models.ModelPrice{
		"provider-model": {InputCostPerToken: 0.000001, OutputCostPerToken: 0.000004},
	})
...
func TestBuildSpendEntryWithoutContextOrPriceIsIneligibleAndNeverUsesCredentialAsTeam(t *testing.T) {
...
func TestBuildSpendEntryWithoutUsageIsFinanciallyIneligible(t *testing.T) {
	registry := models.NewModelPriceRegistry()
	registry.Update(map[string]*models.ModelPrice{
		"provider-model": {InputCostPerToken: 0.000001, OutputCostPerToken: 0.000004},
	})
...
func TestBuildSpendEntryWithMissingPriceIsFinanciallyIneligible(t *testing.T) {
	p := &Proxy{logger: slog.New(slog.DiscardHandler)}
	identity := shadowcontext.Identity{
		APIKeyHash: testClientKeyHash, PublicModel: "public-model", DeploymentID: "deployment-1", CallID: "call-1",
	}
...
func TestBuildSpendEntryMissingPriceIncrementsCounter(t *testing.T) {
	missingModel := func() *litellmdb.SpendLogEntry {
		...
		billing := NewBillingContext("event-1", "call-1", "/v1/chat/completions", identity).
			WithRouting("backend-model", "unpriced-provider-model", "openai", "credential-1", "https://provider.example.invalid/v1")
		return p.buildSpendEntry(&RequestLogContext{
			RequestID:     "event-1",
			CallID:        "call-1",
			ShadowContext: shadowcontext.Result{State: shadowcontext.StateValid, Identity: identity},
			Billing:       billing,
	...
	before := testutil.ToFloat64(monitoring.SpendPriceMissingTotal.WithLabelValues("missing_registry"))
	entry := missingModel()
	require.NotNil(t, entry)
	assert.Zero(t, entry.Spend)
	after := testutil.ToFloat64(monitoring.SpendPriceMissingTotal.WithLabelValues("missing_registry"))
	...
	billing := NewBillingContext("event-2", "call-2", "/v1/chat/completions", identity).
		WithRouting("backend-model", "provider-model", "openai", "credential-1", "https://provider.example.invalid/v1")
	pricedBefore := testutil.ToFloat64(monitoring.SpendPriceMissingTotal.WithLabelValues("missing_registry"))
	pricedEntry := p.buildSpendEntry(&RequestLogContext{
		RequestID:     "event-2",
		CallID:        "call-2",
		ShadowContext: shadowcontext.Result{State: shadowcontext.StateValid, Identity: identity},
		Billing:       billing,
	...
	pricedAfter := testutil.ToFloat64(monitoring.SpendPriceMissingTotal.WithLabelValues("missing_registry"))
	assert.Equal(t, pricedBefore, pricedAfter, "priced model must not increment the missing-price counter")
}

func TestBuildSpendEntryPriceSnapshotUsesResolvedBackendFallback(t *testing.T) {
	registry := models.NewModelPriceRegistry()
...
func TestBuildSpendEntryTokenPricedImageWithoutUsageIsIneligible(t *testing.T) {
	registry := models.NewModelPriceRegistry()
	registry.Update(map[string]*models.ModelPrice{
...
func TestBuildSpendEntryMatchesGoldenCategoricalFields(t *testing.T) {
	type rawRow struct {
	...
	files, err := filepath.Glob("../../testdata/golden/spend-log/*.json")
