...
func TestPostgresSink_PostgreSQLLiteLLMContract(t *testing.T) {
	...
	schemaName := "air_spend_it_" + strings.ReplaceAll(uuid.NewString(), "-", "")
...
func installIntegrationSchema(t *testing.T, ctx context.Context, conn *pgx.Conn) {
	t.Helper()
	schemaSQL, err := os.ReadFile("testdata/litellm_spend_schema.sql")
	require.NoError(t, err)
